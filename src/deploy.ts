...
async function main() {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (isStaleUtxo) {
        console.log('\n  ❌ Stale wallet state detected (UTXO already spent on-chain).');
        console.log('  Fix: clear the wallet cache and re-run deploy:\n');
        console.log('    rm -rf .midnight-wallet-state');
        console.log('    npm run deploy -- --network ' + network + '\n');
        await walletCtx.wallet.stop();
        process.exit(1);
      }
